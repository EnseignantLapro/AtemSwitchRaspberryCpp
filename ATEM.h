...
class ATEM
{
  private:
	...
	SOCKADDR_IN _switcherIP;		// IP address of the switcher
	...
	uint16_t _ClientPakID=0;				// Used when parsing packets
	uint16_t _packetType;  //0 PingACK //1 Init //2Retry //3Hello //4 AWSer
  ...
  public:
    ...
    ATEM();
    ATEM(SOCKADDR_IN ip, const uint16_t localPort);
    void begin(SOCKADDR_IN ip);
    bool connect();
	void deconnect();
    void runLoop();
	//JLA
